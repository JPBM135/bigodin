...
5. **[triple-mustache-and-ampersand.md](triple-mustache-and-ampersand.md)** — option A (recognize the syntax as raw-emit) is a tiny parser change since Bigodin already emits raw. Cheap fix for ~21 spec tests. The HTML-escape side (option B) is a separable, opt-in concern.
...
8. **[inheritance.md](inheritance.md)** — optional spec. The `$` sigil collides directly with Bigodin's existing variable syntax (`{{$this}}`, `{{$parent}}`); resolving that requires context-sensitive parsing. Largest effort.
9. **[lambdas.md](lambdas.md)** — optional spec. Bigodin's `addHelper` is the deliberate, safer alternative; re-parsing template strings produced by user data cuts against Bigodin's "safely run user templates" framing.
