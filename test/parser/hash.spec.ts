...
describe('parser', () => {
    describe('hash arguments', () => {
        ...
        it('rejects positional after hash', () => {
            expect(() => parse('helper k=1 oops')).toThrow(/positional parameters cannot follow hash arguments/i);
        });

        it('rejects positional subexpression after hash', () => {
            expect(() => parse('helper k=1 (foo)')).toThrow(/positional parameters cannot follow hash arguments/i);
        });
