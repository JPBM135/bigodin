...
function removeLoc(obj: any, stringMapper: (v: string) => string = (v) => v): any {
  ...
  if (Array.isArray(obj)) {
    return obj.map((objToRemove) => removeLoc(objToRemove, stringMapper));
  }
...
describe('parser', () => {
  describe('blocks', () => {
    ...
    it('should allow path expressions starting with else', () => {
      ...
      const baselineNoLoc = removeLoc(baseline, (str) => (str === 'elzemar' ? 'elsemar' : str));

      expect(removeLoc(noSpaces)).toEqual(baselineNoLoc);
      expect(removeLoc(spaces)).toEqual(baselineNoLoc);
    });
    ...
    it('should allow path expressions with parameters starting with else', () => {
      ...
      expect(removeLoc(elseExpression)).toEqual(baselineNoLoc);
    });
...
{{/is}}
      ...
      expect((await template({ value: 'one' })).trim()).toEqual('first');
      expect((await template({ value: 'two' })).trim()).toEqual('second');
      expect((await template({ value: 'three' })).trim()).toEqual('third');
      expect((await template({ value: 'four' })).trim()).toEqual('fourth');
      expect((await template({ value: 'five' })).trim()).toEqual('fifth');
    });
...
{{/is}}
      ...
      expect((await template({ value: 'one' })).trim()).toEqual('first');
      expect((await template({ value: 'two' })).trim()).toEqual('second');
      expect((await template({ value: 'three' })).trim()).toEqual('nth');
      expect((await template({ value: 'four' })).trim()).toEqual('nth');
    });

    it('should not parse literals', () => {
      expect(() => parse('{{#"foo"}}')).toThrow(/literal blocks are not allowed/i);
      expect(() => parse('{{/"foo"}}')).toThrow(/literal blocks are not allowed/i);
    });

    it('should not parse unclosed blocks', () => {
      expect(() => parse('{{#foo}}')).toThrow(/make sure this block was closed/i);
    });

    it('should not parse block mismatch', () => {
      expect(() => parse('{{#foo}}{{/bar}}')).toThrow(/this block was opened as/i);
    });

    it('should not parse closing blocks with parameters', () => {
      expect(() => parse('{{#foo}}{{/foo bar}}')).toThrow(/closing blocks cannot have parameters/i);
    });

    it('should not parse else blocks outside blocks', () => {
      expect(() => parse('{{else}}')).toThrow(/{{else}} can only exist inside blocks/i);
    });

    it('should not parse more than one else blocks', () => {
      expect(() => parse('{{#foo}}{{else}}{{else}}{{/foo}}')).toThrow(
        /an {{else}} block was already defined/i,
      );
    });

    it('should not parse else blocks with parameters', () => {
      expect(() => parse('{{#foo}}{{else "bar"}}{{/foo}}')).toThrow(
        /{{else}} blocks cannot have parameters/i,
      );
    });
  });
});
