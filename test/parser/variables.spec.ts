  ...
  describe('variables', () => {
    describe('assignment', () => {
      ...
      it('should not allow assignment to $this', () => {
        expect(() => parse('{{= $this 42}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= $this.foo 42}}')).toThrow(/expected variable/i);
      });

      it('should not allow assignment to $parent', () => {
        expect(() => parse('{{= $parent 42}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= $parent.foo 42}}')).toThrow(/expected variable/i);
      });

      it('should not allow assignment to $root', () => {
        expect(() => parse('{{= $root 42}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= $root.foo 42}}')).toThrow(/expected variable/i);
      });

      it('should not allow assignment without variable name', () => {
        expect(() => parse('{{= 42}}')).toThrow(/expected variable/i);
      });

      it('should not allow assignment without expression', () => {
        expect(() => parse('{{= $foo}}')).toThrow(
          /expected literal, variable, path or parenthesized expression/i,
        );
      });

      it('should not allow assignment with multiple expressions', () => {
        expect(() => parse('{{= $foo 42 24}}')).toThrow(/assignments require a single expression/i);
      });

      it('should not allow assignment to literals', () => {
        expect(() => parse('{{= "foo" 42}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= 42 24}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= true 42}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= false 42}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= null 42}}')).toThrow(/expected variable/i);
        expect(() => parse('{{= undefined 42}}')).toThrow(/expected variable/i);
      });
    });
    ...
    describe('usage', () => {
      ...
      it('should distinguish variables from path discrimination', () => {
        const validNames = ['$this', '$parent', '$root', '$this.foo', '$parent.foo', '$root.foo'];
        for (const name of validNames) {
          const result = parse(`{{ ${name} }}`);
          expect((result.statements[0] as any).expression.type).toEqual('EXPRESSION');
        }
      });

      it('should handle variables starting with context discrimination names', () => {
        const validNames = ['$thisx', '$parentx', '$rootx'];
        for (const name of validNames) {
          const result = parse(`{{ ${name} }}`);
          expect((result.statements[0] as any).expression.type).toEqual('VARIABLE');
          expect((result.statements[0] as any).expression.name).toEqual(name);
        }
      });

      it('should handle valid variable names', () => {
        const validNames = ['$var123', '$foo_bar', '$fooBar', '$_var', '$foo1_1bar123', '$1234'];
        for (const name of validNames) {
          const result = parse(`{{ ${name} }}`);
          expect((result.statements[0] as any).expression.type).toEqual('VARIABLE');
          expect((result.statements[0] as any).expression.name).toEqual(name);
      ...
      it('should not allow variables blocks', () => {
        expect(() => parse('{{#$foo}}{{/$foo}}')).toThrow(/Variable blocks are not allowed/);
        expect(() => parse('{{^$foo}}{{/$foo}}')).toThrow(/Variable blocks are not allowed/);
        expect(() => parse('{{/$foo}}')).toThrow(/Variable blocks are not allowed/);
        expect(() => parse('{{#x}}{{else $foo}}{{/x}}')).toThrow(
          /blocks cannot have variable parameters/,
        );
      });
    });
  });
