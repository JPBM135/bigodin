...
import { parse, parseExpression } from '../../src';
...
describe('parser', () => {
  ...
  it('should combine text, comments and mustaches', () => {
    const text = ['{{! Greeting user }}', 'Hello {{atIndex (split name) 0}}!'].join('\n');
    const result = parse(text);
    expect(result).toEqual({
      type: 'TEMPLATE',
      loc: { start: 0, end: text.length },
  ...
  it('should fail on mustache inside mustache', () => {
    ...
    expect(() => parse(text)).toThrow(/column 9/);
  });

  it('should fail on unexpected close mustache', () => {
    const text = 'some text}}';
    expect(() => parse(text)).toThrow(/column 10/);
  });

  it('should fail on parsing non string', () => {
    const template = 1;
    expect(() => parse(template as any)).toThrow('Template must be a string');
  });

  it('should fail on parsing non string expression', () => {
    const expression = 1;
    expect(() => parseExpression(expression as any)).toThrow('Expression must be a string');
  });

  it('should keep non-whitespace text after a comment without trailing newline', () => {
    const text = '{{! c }}xyz';
    const result = parse(text);
    expect(result.statements).toHaveLength(2);
    expect(result.statements[0].type).toEqual('COMMENT');
  ...
  it('should strip trailing whitespace after a standalone comment at end of template', () => {
    const text = '{{! c }}   ';
    const result = parse(text);
    expect(result.statements).toHaveLength(2);
    expect((result.statements[1] as any).value).toEqual('');
  });
});
