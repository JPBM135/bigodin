...
describe("runner", () => {
  ...
  describe("mustache", () => {
    ...
    it("should return deep path expressions", async () => {
      ...
      expect(
        await templ({ name: { first: "George", last: "Schmidt" } }),
      ).toEqual("Hello, George Schmidt!");
      expect(await templ()).toEqual("Hello,  !");
      expect(await templ({})).toEqual("Hello,  !");
      expect(await templ({ name: null })).toEqual("Hello,  !");
    ...
    it("should allow $this for helper-path disambiguisation", async () => {
      ...
      expect(await templ({ foo: "bar", obj: { deep: "baz" } })).toEqual(
        "bar baz",
      );
    });
    ...
    it("should ignore unsafe keys", async () => {
      const templ = compile(
        "Hello, {{ name.constructor }} {{ name.__proto__ }}!",
      );
      expect(
        await templ({
          name: {
    ...
    it("should not return Object prototype keys", async () => {
      const templ = compile(
        "{{ hasOwnProperty }}{{ obj.toString }}{{ obj.hasOwnProperty }}",
      );
  ...
  describe("expressions", () => {
    ...
    it("should evaluate more complex expression as true", async () => {
      ...
      bigodin.addHelper("startsWith", (s: any, p: any) =>
        String(s).startsWith(p),
      );
      bigodin.addHelper("and", (...args: any[]) => args.every(Boolean));
      const templ = bigodin.compileExpression(
        'and (startsWith foo "b") (eq fruit "apple")',
      );
  ...
  describe("time limit", () => {
    ...
    it("should not interrupt when execution takes less than limit", async () => {
      ...
      expect(
        await template({ foo: "bar" }, { maxExecutionMillis: 50 }),
      ).toEqual("Hello, bar");
    });
