...
describe('runner', () => { describe('blocks', () => {
    ...
    it('should pass array object items as context', async () => {
        const templ = compile('{{#val}}{{foo}}{{/val}}');
        expect(await templ({ val: [{ foo: 'bar' }, { foo: 'baz' }], foo: 'wrong' })).toEqual('barbaz');
    });
    ...
    it('should let access $root for two levels', async () => {
        ...
        expect(await templ({ val: { other: { foo: 'inside' }, foo: 'middle' }, foo: 'outside' })).toEqual('inside outside');
    });
    ...
    it('should let access $parent for two levels', async () => {
        ...
        expect(await templ({ val: { other: { foo: 'inside' }, foo: 'middle' }, foo: 'outside' })).toEqual('inside middle outside');
    });
