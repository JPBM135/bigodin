...
2. **Truthy scalar context:** The final branch renders the body but does not push the scalar value onto the context stack. Mustache wants `{{#foo}}` over `foo: "bar"` to push `"bar"` so `{{.}}` resolves to it. (Same fix is recommended in [implicit-iterator.md](/docs/mustache-compat/implicit-iterator); this doc is the place to track the *runtime* change while implicit-iterator covers the *parser* change.)
